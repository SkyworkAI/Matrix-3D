...
class FluxFillPipeline(
    ...
    def prepare_mask_latents(
        ...
        print(f"Before origin masked_image_latents={masked_image_latents.shape}")
        ...
        print(f"origin masked_image_latents={masked_image_latents.shape}")
        ...
        for x in range(blend_extent):
            b[:, :, :, x] = a[:, :, :, -blend_extent
                              + x] * (1 - x / blend_extent) + b[:, :, :, x] * (
                                  x / blend_extent)
        # a_blend = a[:, :, :, -blend_extent:]  # a的右侧混合区
        # b_blend = b[:, :, :, :blend_extent]   # b的左侧混合区
        # weights = torch.linspace(1.0, 0.0, blend_extent, device=a.device)  # 从1渐变到0
        # b[:, :, :, :blend_extent] = a_blend * weights + b_blend * (1 - weights)
        return b
    ...
    def __call__(
        ...
        if masked_image_latents is not None:
            ...
            masked_image_latents = torch.cat((masked_image_latents, mask), dim=-1)#?这里进行了合并，一共是5倍channel
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i > len(timesteps) * 0.6:
                    ...
                    self.swap_history.append(rand_width)
                    ...
                    masked_latents_unpack = self._unpack_latents(masked_image_latents, height, width_new_blended * self.vae_scale_factor, self.vae_scale_factor)
                    masked_latents_unpack=self.swap_left_right(masked_latents_unpack,rand_width=rand_width)
                    # masked_latents_unpack=self.blend_h(masked_latents_unpack, masked_latents_unpack, 4)
                    latent_height = 2 * (int(height) // (self.vae_scale_factor * 2))
                    num_channels_latents_mask = masked_latents_unpack.shape[1]
                    masked_image_latents = self._pack_latents(masked_latents_unpack, batch_size, num_channels_latents_mask, latent_height, width_new_blended)
        ...
        ##?restore
        if hasattr(self, 'swap_history'):
            for rand_width in reversed(self.swap_history):
                 latents_unpack = self.swap_left_right(latents_unpack,rand_width=rand_width,is_swap=False)
    ...
    def swap_left_right(self,latents_unpack,pad_ratio=0.1,rand_width=0, is_swap=True):
        B,C,H,W = latents_unpack.shape
        pad_size = int(W*pad_ratio)
        latents_padded = F.pad(latents_unpack, (pad_size, pad_size,0,0),mode='circular')

        rand_width = rand_width if is_swap else W-rand_width
        if rand_width > 0:
            assert rand_width < W, f"rand_width ({rand_width}) must be < image width ({W}). Check input image dimensions."
