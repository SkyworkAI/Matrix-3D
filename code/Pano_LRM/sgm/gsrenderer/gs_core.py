...
def compute_dirs_from_xyz(xyz: torch.Tensor) -> torch.Tensor:
    cam_pos = torch.zeros((1, 3), device=xyz.device)
    dirs = xyz - cam_pos[:, None, :]
    dirs = dirs / dirs.norm(dim=-1, keepdim=True)
    return dirs  # (B, N, 3)

def batch_sh_rgb(sh_tensor: torch.Tensor, xyz: torch.Tensor) -> torch.Tensor:
    """
    Args:
        sh_tensor: (B, N, 3, 9) SH coefficients
        xyz: (B, N, 3) point locations in world

    Returns:
        rgb: (B, N, 3)
    """
    assert sh_tensor.shape[-2:] == (3, 9), "SH tensor must be (B, N, 3, 9)"
    dirs = compute_dirs_from_xyz(xyz)  # (B, N, 3)

    r = eval_sh(2, sh_tensor[:, :, 0, :], dirs)  # (B, N)
    g = eval_sh(2, sh_tensor[:, :, 1, :], dirs)  # (B, N)
    b = eval_sh(2, sh_tensor[:, :, 2, :], dirs)  # (B, N)

    rgb = torch.stack([r, g, b], dim=-1)  # (B, N, 3)
    return rgb
def RGB2SH(rgb):
    return (rgb - 0.5) / C0
